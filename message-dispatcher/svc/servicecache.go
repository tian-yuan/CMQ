...
/*
The HandlerFunc type is the service update function,
will be called when services is updated
services is nil when all services is down
 */
type HandlerFunc func(service string, services []*registry.Service)

// ServiceCache is the registry cache interface
...
func (c *cache) set(service string, services []*registry.Service) {
	...
	c.handler(service, services)
}

func (c *cache) update(res *registry.Result) {
	if res == nil || res.Service == nil {
		log.Errorf("update res is null.")
		return
	}
	...
	if !ok {
		...
		log.Info("not any lookup")
		return
	}

	log.Infof("handle cache update, service name : %s, len : %s", res.Service.Name, len(services))
...
func New(r registry.Registry, hander HandlerFunc, opts ...Option) ServiceCache {
